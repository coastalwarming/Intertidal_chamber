  ...
  while (file.available())    // note how this also prevents the buffer from overflowing (18 max to leave space for '\0'!)
  {
    ...
    if (file.peek() == '\n') {
      ...
      if (strstr(buffer, "=") > 0) {
        ...
        //Chiller_wait:
        if (strstr(buffer, "Chiller_wait=") > 0) {
          p = strtok(buffer, "=");
          p = strtok(NULL, "=");
          if (p != NULL) {
            Chiller_wait = atoi(p);
        ...
        //Top_sensors:
        if (strstr(buffer, "Top_sensors=") > 0) {
          p = strtok(buffer, "=");
          p = strtok(NULL, "=");
          if (p != NULL) {
        ...
        //Bottom_sensors:
        if (strstr(buffer, "Bottom_sensors=") > 0) {
          p = strtok(buffer, "=");
          p = strtok(NULL, "=");
          if (p != NULL) {
        ...
        //Water_release:
        if (strstr(buffer, "Water_release=") > 0) {
          p = strtok(buffer, "=");
          p = strtok(NULL, "=");
          if (p != NULL) {
        ...
        */
        ...
        //auth:
        if (strstr(buffer, "BLYNK_AUTH_TOKEN") > 0) {
          p = strtok(buffer, "=");
          p = strtok(NULL, "=");
          if (p != NULL) {
...
void Create_n_Log_data_to_SD() {
  ...
  */
  ...
  if (!Reset) {
    appendFile(SD, File_name , "Reset", 1);
    Reset = true;
  }

  if (sd_back) {
    appendFile(SD, File_name , "SD reconnected", 1);
    sd_back = false;
  }
  ...
  appendFile(SD, File_name , Log_data, 1);
...
bool SenseSD(void) {
  ...
  if (SD.open("/test.txt", "w"))
  {
    ...
    if (!sd_exist) {
      ...
      xSemaphoreGive(mutex);
      Did_Profile_changed(SD, "/Profile.txt");
      sd_exist = true;
      sd_back = true;
    }
...
void start_sd() {
  if (InitSDCard()) {
    ...
    File file_2 = SD.open("/test_2.txt", FILE_WRITE);
...
bool InitSDCard() {
  ...
  if (!SD.begin()) {
    for (int i = 0; i < 10; i++) {
      // if (SD.begin(5, spi, 8000000))return 1; else {
      if (SD.begin())return 1; else {
        SD.end();
