...
void restartMCU() {
  Serial.println("Vou fazer reset em ESP.restart()");
  delay(100);
  ESP.restart();
  while(1) {};
}
...
void getWiFiName(char* buff, size_t len, bool withPrefix = true) {
  ...
  if (withPrefix) {
    snprintf(buff, len, "Blynk %s-%05X", BLYNK_DEVICE_NAME_, unique);//Troquei BLYNK_DEVICE_NAME por BLYNK_DEVICE_NAME_ .
  } else {
    snprintf(buff, len, "%s-%05X", BLYNK_DEVICE_NAME_, unique);//Troquei BLYNK_DEVICE_NAME por BLYNK_DEVICE_NAME_ .
  }
}
...
{
  Serial.println("MEU: enterConfigMode");
  char ssidBuff[64];
  ...
  server.on("/board_info.json", []() {
    ...
    const char *tmpl = BLYNK_TEMPLATE_ID_;//Troquei BLYNK_TEMPLATE_ID por BLYNK_TEMPLATE_ID_
    char ssidBuff[64];
    getWiFiName(ssidBuff, sizeof(ssidBuff));
    char buff[512];
    snprintf(buff, sizeof(buff),
             R"json({"board":"%s","tmpl_id":"%s","fw_type":"%s","fw_ver":"%s","ssid":"%s","bssid":"%s","last_error":%d,"wifi_scan":true,"static_ip":true})json",
             BLYNK_DEVICE_NAME_,//Troquei BLYNK_DEVICE_NAME por BLYNK_DEVICE_NAME_ .
             tmpl ? tmpl : "Unknown",
             BLYNK_FIRMWARE_TYPE,
...
void enterConnectNet() {
  ...
  /*
                                      snprintf_P(log_1, sizeof(log_1), PSTR("Connecting to WiFi .."));Serial.print(log_1);
                                      //Serial.printf_P(PSTR("Connecting to WiFi ..\n"));
                                      //Serial.print("Connecting to WiFi ..");
                                      byte cnt=0;
                                      while ((WiFi.status() != WL_CONNECTED)&&(cnt<10)) {
                                         esp_task_wdt_reset();
                                        snprintf_P(log_1, sizeof(log_1), PSTR("%d...."),cnt);Serial.print(log_1);
                                        //Serial.printf_P(PSTR("%d....\n"),cnt);
                                        //Serial.printf("%d....",cnt);
                                        delay(1000);
                                        cnt++;
                                      }
                                      if ((WiFi.status() == WL_CONNECTED)&&(cnt>=10)) Online=true;
                                    */
...
void enterError() {
  ...
  Serial.println("Entrei em modo erro -> store time and flag");
  Falhei_Blynk_login_time  = millis()+60000;
  Falhei_Blynk_login_flag =true;
  ...
  /*
                      ...
                      { Serial.println("Estou aqui preso.");
                        delay(10);
                        app_loop();
                      ...
                      Serial.println("Cheguei ao reset");
                      //Blynk_connected = false;
                      //restartMCU();

                      teste_5++;
                       if(teste_5==5){ BlynkState::set(MODE_CONNECTING_CLOUD);
                       Serial.println("teste_5==5){ BlynkState::set(MODE_CONNECTING_CLOUD);");}
                       */
}
