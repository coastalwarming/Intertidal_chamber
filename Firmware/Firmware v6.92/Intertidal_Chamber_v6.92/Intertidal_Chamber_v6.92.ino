...
const byte Sensor_4_Pin = 17;
const byte Sensor_5_Pin = 25;
const byte Sensor_6_Pin = 13;
...
//Luz
  ledcSetup(0, 100, 8);//ledChannel, freq, resolution
  ledcAttachPin(LightsPin, 0);
 ...
 //
  ...
  /*
    xTaskCreatePinnedToCore(
     Request_Air_Temperatures,
     "Task2",// nome da tarefa
     10000, // número de palavras a serem alocadas para uso com a pilha da tarefa
     NULL,// parâmetro de entrada para a tarefa (pode ser NULL)
     1,// prioridade da tarefa (0 a N)
     &Task3,// referência para a tarefa (pode ser NULL)
...
/*
  ...
  xTaskCreatePinnedToCore(
    MQTT_Connect_Task_1,
    "MQTT_Connect_Task_1",// nome da tarefa 
    5000, // número de palavras a serem alocadas para uso com a pilha da tarefa 
    NULL,// parâmetro de entrada para a tarefa (pode ser NULL) 
    tskIDLE_PRIORITY,// prioridade da tarefa (0 a N) //tskIDLE_PRIORITY é prioridade 0, a menor de todos, sera a ultima a correr.
    &MQTT_Connect_Task_1_Handle,// referência para a tarefa (pode ser NULL) 
    0);// Núcleo que executará a tarefa 
*/
      //NTP_Task
  xTaskCreatePinnedToCore(
...
//Task 6 Blynk
  xTaskCreatePinnedToCore(
    Ligar_Blynk_Task_6,   // função que implementa a tarefa 
    "Ligar_Blynk_Task_6", // nome da tarefa 
    10000,      // número de palavras a serem alocadas para uso com a pilha da tarefa 
    NULL,       // parâmetro de entrada para a tarefa (pode ser    'NULL,'   ou    '(void *)&qualquernome),'    
    2,          // prioridade da tarefa (0 a N) 
    &Ligar_Blynk_Task_6_Handle,       // referência para a tarefa (pode ser NULL) 
    0);         // Núcleo que executará a tarefa 
    //
    */
  //SD:
  ...
  if (sd_exist){
    updateFromFS(SD);
    Profile_ok = GetProfile_SD(SD, "/Profile.txt");
    readFile(SD, "/Settings.txt");
    readFile_KILL(SD, "/KILL.txt");
    }
  read_settings=true;
