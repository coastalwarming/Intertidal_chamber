...
void Apply_Light() {
  ...
  if (runStatus == Ongoing ) {
    ...
    if (Main_SW_State == Auto) {
      ...
      //Serial.print("Now_Light:");Serial.println(Now_Light);
      //ledcWrite(LightsPin, Now_Light);
      ledcWrite(LightsPin, map(Now_Light, 0, 100, 0, 255));//mcp_20.digitalWrite(5, Now_Light);//mcp_20.5 = 12v_1 = Lights
    }

    if (Main_SW_State  == Manual && Light_sw_state  == Auto) {
      // pinMode(21, OUTPUT);
      // digitalWrite(21, HIGH);
      ledcWrite(LightsPin, map(Now_Light, 0, 100, 0, 255));//mcp_20.digitalWrite(5, Now_Light);//mcp_20.5 = 12v_1 = Lights
    }
  ...
  if (Main_SW_State == Auto && runStatus != Ongoing) {
    ...
    Now_Light = 0;
    ledcWrite(LightsPin, 0); //mcp_20.digitalWrite(5, LOW);
  }
  if (Main_SW_State == Manual && Light_sw_state == ON) {

    Now_Light = 100;
    ledcWrite(LightsPin, map(Now_Light, 0, 100, 0, 255)); //mcp_20.digitalWrite(5, HIGH);
  }
